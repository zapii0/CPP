...
void		ClapTrap::attack(const std::string& target)
{
	if (_energy > 0)
	{
		std::cout << "Clap Trap " << _name << "attacks " << target << ", causing " << _attack_dmg << "points of damage\n";
		_energy--;
...
void		ClapTrap::takeDamage(unsigned int amount)
{
	...
	std::cout << "Clap trap has " << _health << "HP left\n";
}
...
void		ClapTrap::beRepaired(unsigned int amount)
{
	...
	else
	{
		_health += amount;
		_energy--;
		std::cout << "ClapTrap " << _name << "repaired 1 point of HP from 1 points of energy\n";
	}
}
ClapTrap::~ClapTrap()
{
